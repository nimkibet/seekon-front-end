...
const Register = () => {
  ...
  const handleGoogleSignup = async (credentialResponse) => {
    try {
      ...
      const response = await axios.post(`${import.meta.env.VITE_API_URL || 'https://api.seekonapparelglobal.com'}/api/auth/google`, {
  ...
  const handleSendVerificationCode = async () => {
    ...
    try {
      const response = await axios.post(`${import.meta.env.VITE_API_URL || 'https://api.seekonapparelglobal.com'}/api/auth/send-code`, {
        email
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${import.meta.env.VITE_API_URL || 'https://api.seekonapparelglobal.com'}/api/auth/register`, {
      ...
      if (response.data.success) {
        ...
        toast.success('Account created successfully!', { id: 'register-submit' });
      }
    } catch (err) {
          ...
          >
            ...
            {isRegistered ? (
              ...
              <form className="space-y-3" onSubmit={handleSubmit}>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={handleChange}
                      disabled={isCodeSent}
                      className={`w-full pl-9 sm:pl-10 pr-9 sm:pr-10 py-2.5 sm:py-3 text-sm sm:text-base bg-white/20 backdrop-blur-md border border-white/30 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#00A676]/50 focus:border-[#00A676] text-gray-900 dark:text-white placeholder:text-gray-600 dark:placeholder:text-gray-400 ${errors.email ? 'border-red-500 focus:ring-red-500' : ''} ${isCodeSent ? 'opacity-60' : ''}`}
                ...
                {isCodeSent && (
                  <div className="mt-2">
                    <label htmlFor="verificationCode" className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">
                      Verification Code
                    </label>
                    <div className="relative">
                      <input
                        ...
                        onChange={handleChange}
                        className={`w-full py-2.5 sm:py-3 text-sm sm:text-base bg-white/20 backdrop-blur-md border border-white/30 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#00A676]/50 focus:border-[#00A676] text-gray-900 dark:text-white placeholder:text-gray-600 dark:placeholder:text-gray-400 text-center tracking-widest font-mono ${errors.verificationCode ? 'border-red-500 focus:ring-red-500' : ''}`}
                        placeholder="Enter 6-digit code"
                      />
                    </div>
                    {errors.verificationCode && (
                      <p className="mt-0.5 text-[10px] text-red-600 dark:text-red-400">{errors.verificationCode}</p>
                    )}
                    <button
                      type="button"
                      onClick={handleSendVerificationCode}
