...
const AllAccessories = () => {
  ...
  // Helper function to filter out flash sale products
  const isFlashSaleProduct = (p) => 
    p.isFlashSale || p.onFlashSale || p.isOnFlashSale || 
    (p.flashSalePrice && p.flashSalePrice > 0 && p.flashSalePrice < p.price);

  useEffect(() => {
    const accessories = products
      .filter(p => p.category === 'Accessories' || p.category === 'accessories')
      .filter(p => !isFlashSaleProduct(p)); // Exclude flash sale products
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 pt-20">
      ...
      <div className="container mx-auto px-4 py-4 border-t border-b border-gray-200 dark:border-gray-700">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <button
            ...
            className="flex items-center space-x-2 text-gray-900 dark:text-gray-100 hover:text-[#00A676] transition-colors"
          >
          ...
          <div className="relative">
            <select 
              ...
              className="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 text-sm sm:text-base z-50 relative"
      ...
      <div 
        className={`container mx-auto px-4 overflow-hidden transition-all duration-300 ${
          isFilterOpen ? 'max-h-[1000px] py-6' : 'max-h-0 py-0'
        }`}
      >
