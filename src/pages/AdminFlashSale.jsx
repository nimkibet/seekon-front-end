// Flash Sale Management Component
// React imports verified and present
...
const AdminFlashSale = () => {
  ...
  return (
    <div className="p-6">
      {/* FORCE UPDATE V4 */}
      {/* Banner */}
      <div className="bg-gradient-to-r from-red-600 via-red-500 to-orange-500 rounded-2xl p-6 sm:p-8 mb-8">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-xl bg-white/20 backdrop-blur flex items-center justify-center">
              <FiZap className="w-7 h-7 sm:w-8 sm:h-8 text-white" />
            </div>
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-white mb-1">
                Flash Sale Management
              </h1>
              <p className="text-white/80 text-sm sm:text-base">Manage flash sale prices and schedules for all products</p>
            </div>
          </div>
          {/* Banner Stats */}
          <div className="flex items-center gap-3 mt-4 sm:mt-0">
            <div className="bg-white/20 backdrop-blur rounded-lg px-3 py-2 text-white text-center">
              <p className="text-xs opacity-80">Active</p>
              <p className="font-bold text-lg">{activeFlashSales}</p>
            </div>
            <div className="bg-white/20 backdrop-blur rounded-lg px-3 py-2 text-white text-center">
              <p className="text-xs opacity-80">On Sale</p>
              <p className="font-bold text-lg">{products.filter(p => p.isFlashSale).length}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <motion.div
        ...
        className="bg-gradient-to-br from-gray-800/80 to-gray-900/80 backdrop-blur-xl rounded-xl p-6 sm:p-8 border border-white/10 mb-8"
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="flex items-center justify-between p-5 bg-black/30 rounded-xl border border-white/10">
            <div>
              <h3 className="text-white font-semibold mb-1">Flash Sale Status</h3>
            ...
            <button
              ...
              className={`relative inline-flex h-10 w-16 sm:h-8 sm:w-14 items-center rounded-full transition-colors ${
                globalSettings.isActive ? 'bg-[#00A676]' : 'bg-gray-600'
              }`}
            >
              <span
                className={`inline-block h-7 w-7 sm:h-6 sm:w-6 transform rounded-full bg-white transition-transform ${
                  globalSettings.isActive ? 'translate-x-7 sm:translate-x-7' : 'translate-x-1'
                }`}
              />
            </button>
          </div>
          ...
          <div className="p-5 bg-black/30 rounded-xl border border-white/10">
            <h3 className="text-white font-semibold mb-2">Flash Sale End Time</h3>
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-gray-800/50 border border-gray-600 rounded-lg py-2.5 sm:py-2 pl-10 pr-4 text-white focus:border-[#00A676] focus:outline-none transition-colors"
              />
            </div>
          </div>
        </div>
      ...
      <div className="flex justify-end mt-4 sm:mt-6">
        <button
          ...
          className="w-full sm:w-auto bg-[#00A676] hover:bg-[#008A5E] text-white font-bold py-3 px-8 rounded-xl transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {isSavingSettings ? (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="bg-gradient-to-br from-orange-500/20 to-red-500/10 backdrop-blur-xl rounded-xl p-5 border border-orange-500/30"
        >
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-400 text-sm">Active Flash Sales</p>
              <p className="text-4xl font-bold text-orange-400">{activeFlashSales}</p>
            </div>
            <div className="w-14 h-14 rounded-xl bg-orange-500/20 flex items-center justify-center">
              <FiZap className="w-7 h-7 text-orange-400" />
            </div>
          </div>
        </motion.div>
        ...
        <motion.div
          ...
          className="bg-gradient-to-br from-green-500/20 to-emerald-500/10 backdrop-blur-xl rounded-xl p-5 border border-green-500/30"
        >
          <div className="flex items-center justify-between">
            <div>
        ...
        <motion.div
          ...
          className="bg-gradient-to-br from-red-500/20 to-pink-500/10 backdrop-blur-xl rounded-xl p-5 border border-red-500/30"
        >
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-400 text-sm">Total Discounts</p>
              <p className="text-4xl font-bold text-red-400">{totalDiscount}%</p>
            </div>
            <div className="w-14 h-14 rounded-xl bg-red-500/20 flex items-center justify-center">
              <FiTrendingUp className="w-7 h-7 text-red-400" />
            </div>
          </div>
        </motion.div>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="p-4 sm:p-5">
                <h3 className="text-white font-bold text-sm sm:text-base mb-1 truncate">{product.name}</h3>
                ...
                {product.isFlashSale ? (
                  ...
                  <p className="text-[#00A676] font-bold text-lg sm:text-xl mb-3">
                    KSh {product.price?.toLocaleString()}
                  </p>
                )}
                ...
                <div className="flex gap-2 mt-3 sm:mt-4">
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-1 px-3 py-2.5 sm:py-2 bg-[#00A676] hover:bg-[#008A5E] text-white text-sm rounded-lg transition-colors"
                  >
                    <FiEdit className="w-4 h-4" />
                    <span className="hidden sm:inline">{product.isFlashSale ? 'Edit Sale' : 'Set Sale'}</span>
                    <span className="sm:hidden">{product.isFlashSale ? 'Edit' : 'Set'}</span>
                  </button>
                  <button
                    onClick={() => quickToggleFlashSale(product)}
                    className={`px-3 py-2.5 sm:py-2 rounded-lg transition-colors ${
                      product.isFlashSale 
