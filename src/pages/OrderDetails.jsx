...
import { FiArrowLeft, FiCheckCircle, FiClock, FiUser, FiMapPin, FiCreditCard, FiTruck, FiPackage } from 'react-icons/fi';
  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      try {
        ...
        // The API returns { success: true, order }, so we extract the order
          ...
          <div className="p-6 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-6">
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <p className="text-base font-medium text-gray-900 dark:text-gray-100">
                      {order.shippingAddress?.address || 'N/A'}{order.shippingAddress?.city ? `, ${order.shippingAddress.city}` : ''}
                    </p>
                ...
                {/* Delivery Notes - Only show if data exists */}
                {(order.expectedArrival || order.deliveryDetails) && (
                  <div className="mt-4 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                    <h4 className="font-semibold text-sm text-blue-900 dark:text-blue-100 mb-2">Fulfillment Details</h4>
                    {order.expectedArrival && (
                      <p className="text-sm text-blue-700 dark:text-blue-300">
                        <span className="font-medium">ETA:</span> {order.expectedArrival}
                      </p>
                    )}
                    {order.deliveryDetails && (
                      <p className="text-sm text-blue-700 dark:text-blue-300 mt-1">
                        <span className="font-medium">Note:</span> {order.deliveryDetails}
                      </p>
                    )}
                  </div>
                )}
