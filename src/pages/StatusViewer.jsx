...
import { ChevronLeft, ChevronRight, X, Volume2, VolumeX, Loader2 } from 'lucide-react';
...
const StatusViewer = () => {
  ...
  const handleMouseDown = (e) => {
    // Only detect clicks if we are on a mobile device width (on desktop, we use chevrons)
    ...
    return (
      <div className="fixed inset-0 bg-[#0B0F19] flex flex-col items-center justify-center text-white p-6 z-50">
        <div className="max-w-md text-center">
          ...
          <p className="text-gray-400 mb-8 leading-relaxed">Seekon Apparel doesn't have any active status updates right now. Check back later to see our latest drops!</p>
          <button 
            onClick={() => navigate('/')}
  ...
  return (
    <div className="fixed inset-0 bg-black overflow-hidden select-none z-50 flex items-center justify-center">
      ...
      >
        ...
        {/* PERSISTENT CONVERSION CTA BUTTON */}
        <div className="absolute bottom-0 inset-x-0 p-5 bg-gradient-to-t from-black/90 to-transparent z-30 flex justify-center no-tap-navigation">
          <a 
            ...
            className="w-full bg-[#25D366] hover:bg-[#20BA5A] text-white py-3.5 px-6 rounded-full font-bold shadow-lg shadow-[#25D366]/20 transition-all duration-300 transform hover:scale-[1.03] active:scale-[0.97] flex items-center justify-center gap-2.5 text-center text-sm"
          >
